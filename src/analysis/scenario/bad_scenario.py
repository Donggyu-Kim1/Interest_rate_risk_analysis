...
class BadScenarioAnalysis:
    ...
    def analyze_spread_widening(self):
        ...
        for _, bond in self.bond_info.iterrows():
            ...
            # 만기별 스프레드 확대 폭 설정
            if maturity <= 2:
                spread_widening = np.random.uniform(0.002, 0.003)  # 20-30bp
            elif maturity <= 5:
                spread_widening = np.random.uniform(0.003, 0.004)  # 30-40bp
            else:
                spread_widening = np.random.uniform(0.004, 0.005)  # 40-50bp
            ...
            # 스프레드 확대로 인한 가치 변화 추정
            spread_impact = bond["발행액"] * (spread_widening)
    ...
    def calculate_total_impact(self):
        ...
        total_results["상호작용효과"] = (
            total_results["금리충격손실"] * total_results["스프레드손실"] * 0.1
        )
